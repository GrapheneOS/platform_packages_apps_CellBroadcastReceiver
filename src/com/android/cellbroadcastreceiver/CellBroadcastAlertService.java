...
public class CellBroadcastAlertService extends Service {
    ...
    static void addToNotificationBar(SmsCbMessage message,
        ...
        // For FEATURE_WATCH we want to use a unique notification id
        int notificationId = getNotificationId(message, isWatch);
    ...
    /**
     * Calculates the expected notification ID for a given CellBroadcast message.
     *
     * <p>Watches (Wear OS) require a unique notification ID for each alert so they do
     * not overwrite each other. This unique ID is generated by bit-shifting the service
     * category and OR-ing it with the serial number. For non-watch devices, a static
     * default {@code NOTIFICATION_ID} (1) is used.
     *
     * @param message The {@link SmsCbMessage} being processed.
     * @return The expected integer notification ID.
     */
    @VisibleForTesting
    static public int getNotificationId(SmsCbMessage message, boolean isWatch) {
        if (isWatch) {
            return (message.getServiceCategory() << 16) | message.getSerialNumber();
        }
